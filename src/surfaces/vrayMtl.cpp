...
#define LUISA_RENDER_vrayMtl_PARAM_LOAD(name) \
    _##name = scene->load_texture(desc->property_node_or_default(#name));
        LUISA_RENDER_vrayMtl_PARAM_LOAD(metallic)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(eta)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(roughness)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(specular_tint)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(anisotropic)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(sheen)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(sheen_tint)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(clearcoat)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(clearcoat_gloss)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(specular_trans)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(flatness)
        LUISA_RENDER_vrayMtl_PARAM_LOAD(diffuse_trans)
...
class vrayMtlDiffuse final : public BxDF {

private:
    SampledSpectrum R;

public:
    explicit vrayMtlDiffuse(const SampledSpectrum &R) noexcept : R{R} {}
...
class vrayMtlFakeSS final : public BxDF {
...
public:
    vrayMtlFakeSS(const SampledSpectrum &R, Float roughness) noexcept
...
class vrayMtlRetro final : public BxDF {
...
public:
    vrayMtlRetro(const SampledSpectrum &R, Float roughness) noexcept
        : R{R}, roughness{std::move(roughness)} {}
    [[nodiscard]] SampledSpectrum albedo() const noexcept override { return R; }
    [[nodiscard]] SampledSpectrum evaluate(Expr<float3> wo, Expr<float3> wi, TransportMode mode) const noexcept override {
        static Callable impl = [](Float3 wo, Float3 wi, Float roughness) noexcept {
...
// pImpl idiom.
class vrayMtlClosureImplBase {

public:
    virtual ~vrayMtlClosureImplBase() noexcept = default;
...
class vrayMtlSurfaceClosureImpl final : public vrayMtlClosureImplBase {
...
private:
    ...
    luisa::unique_ptr<vrayMtlDiffuse> _diffuse;
    luisa::unique_ptr<vrayMtlFakeSS> _fake_ss;
    luisa::unique_ptr<vrayMtlRetro> _retro;
    luisa::unique_ptr<vrayMtlSheen> _sheen;
    luisa::unique_ptr<vrayMtlMicrofacetDistribution> _distrib;
    luisa::unique_ptr<vrayMtlFresnel> _fresnel;
    luisa::unique_ptr<MicrofacetReflection> _specular;
    luisa::unique_ptr<vrayMtlClearcoat> _clearcoat;
...
public:
    ...
    vrayMtlSurfaceClosureImpl(const Surface::Closure *cls, Expr<float> eta_i,
        ...
        if (color_tex == nullptr || !color_tex->node()->is_black()) {
            ...
            _diffuse = luisa::make_unique<vrayMtlDiffuse>(Cdiff);
            _retro = luisa::make_unique<vrayMtlRetro>(Cdiff, roughness);
            // fake subsurface scattering
            if (flatness_tex && !flatness_tex->node()->is_black()) {
                auto Css_weight = diffuse_weight * flatness;
                auto Css = Css_weight * _color;
                _fake_ss = luisa::make_unique<vrayMtlFakeSS>(Css, roughness);
            }
            ...
            if (sheen_tex && !sheen_tex->node()->is_black()) {
                ...
                _sheen = luisa::make_unique<vrayMtlSheen>(Csheen);
        ...
        _fresnel = luisa::make_unique<vrayMtlFresnel>(
        ...
        _distrib = luisa::make_unique<vrayMtlMicrofacetDistribution>(alpha);
        ...
        if (clearcoat_tex && !clearcoat_tex->node()->is_black()) {
            ...
            _clearcoat = luisa::make_unique<vrayMtlClearcoat>(cc, gloss);
...
class ThinvrayMtlSurfaceClosureImpl : public vrayMtlClosureImplBase {
...
public:
    ThinvrayMtlSurfaceClosureImpl(const Surface::Closure *cls, Expr<float> eta_i,
                                 const Texture::Instance *color_tex, const Texture::Instance *metallic_tex,
                                 const Texture::Instance *eta_tex, const Texture::Instance *roughness_tex,
                                 const Texture::Instance *spec_tint_tex, const Texture::Instance *aniso_tex,
                                 const Texture::Instance *sheen_tex, const Texture::Instance *sheen_tint_tex,
...
private:
    luisa::unique_ptr<vrayMtlClosureImplBase> _impl;
...
public:
    vrayMtlSurfaceClosure(const vrayMtlSurfaceInstance *instance, luisa::shared_ptr<Interaction> it,
        ...
        if (instance->node()->is_thin()) {
            _impl = luisa::make_unique<ThinvrayMtlSurfaceClosureImpl>(
                this, eta_i,
